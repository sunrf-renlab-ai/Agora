...
# Works when `supabase db push` fails because direct TCP to :5432 is
# blocked (Clash/Mihomo fakeip, corporate firewall, ISP transparent
# proxies). HTTPS is almost always allowed.
#
# Usage:
#   SUPABASE_PAT=sbp_… PROJECT_REF=<ref> ./supabase-push-migration.sh path/to/file.sql
#
# Multi-file (manual loop):
#   for f in supabase/migrations/*.sql; do
#     SUPABASE_PAT=sbp_… PROJECT_REF=<ref> \
#       scripts/zero-cost-deploy/supabase-push-migration.sh "$f"
#   done
#
# Or use the wrapper (sorts + iterates agora's migrations directory):
#   SUPABASE_PAT=sbp_… PROJECT_REF=<ref> \
...
: "${PROJECT_REF:?PROJECT_REF not set — find it in your project URL: https://<ref>.supabase.co}"
...
# Build JSON safely with jq (handles quoting/escaping for arbitrary SQL).
